...
# This file hooks up shell integration for wezboard.
# It is suitable for zsh and bash.
#
# Although wezboard is mentioned here, the sequences used are not wezboard
...
#
# WEZBOARD_SHELL_SKIP_ALL - disables all
# WEZBOARD_SHELL_SKIP_SEMANTIC_ZONES - disables zones
# WEZBOARD_SHELL_SKIP_CWD - disables OSC 7 cwd setting
# WEZBOARD_SHELL_SKIP_USER_VARS - disable user vars that capture information
...
if [ "${WEZBOARD_SHELL_SKIP_ALL-}" = "1" ] ; then
  return 0
fi
...
__wezboard_install_bash_prexec() {
...
} # end of __wezboard_install_bash_prexec
...
# command provided by wezboard if wezboard is installed, but falls
# back to a simple printf command otherwise.
__wezboard_osc7() {
  if hash wezboard 2>/dev/null ; then
    wezboard set-working-directory 2>/dev/null && return 0
    # If the command failed (perhaps the installed wezboard
...
__wezboard_semantic_precmd_executing=""
__wezboard_semantic_precmd() {
  local ret="$?"
  if [[ "$__wezboard_semantic_precmd_executing" != "0" ]] ; then
    __wezboard_save_ps1="$PS1"
    __wezboard_save_ps2="$PS2"
    ...
    __wezboard_check_ps1="$PS1"
  fi
  if [[ "$__wezboard_semantic_precmd_executing" != "" ]] ; then
  ...
  __wezboard_semantic_precmd_executing=0
}

function __wezboard_semantic_preexec() {
  # Restore the original PS1/PS2 if set
  if [[ -n "${__wezboard_save_ps1+1}" && "${__wezboard_check_ps1-}" == "${PS1}" ]]; then
	  PS1="$__wezboard_save_ps1"
	  PS2="$__wezboard_save_ps2"
	  unset __wezboard_save_ps1
  fi
  # Indicate that the command output begins here
  printf "\033]133;C;\007"
  __wezboard_semantic_precmd_executing=1
}

__wezboard_user_vars_precmd() {
  __wezboard_set_user_var "WEZBOARD_PROG" ""
  __wezboard_set_user_var "WEZBOARD_USER" "$(id -un)"

  # Indicate whether this pane is running inside tmux or not
  if [[ -n "${TMUX-}" ]] ; then
    __wezboard_set_user_var "WEZBOARD_IN_TMUX" "1"
  else
    __wezboard_set_user_var "WEZBOARD_IN_TMUX" "0"
  fi

  # You may set WEZBOARD_HOSTNAME to a name you want to use instead
  # of calling out to the hostname executable on every prompt print.
if [[ -z "${WEZBOARD_HOSTNAME}" ]]; then
  if [[ -r /proc/sys/kernel/hostname ]]; then
    __wezboard_set_user_var "WEZBOARD_HOST" "$(cat /proc/sys/kernel/hostname)"
  elif hash hostname 2>/dev/null; then
    __wezboard_set_user_var "WEZBOARD_HOST" "$(hostname)"
  elif hash hostnamectl 2>/dev/null; then
    __wezboard_set_user_var "WEZBOARD_HOST" "$(hostnamectl hostname)"
  else
    __wezboard_set_user_var "WEZBOARD_HOST" "unknown"
  fi
else
  __wezboard_set_user_var "WEZBOARD_HOST" "${WEZBOARD_HOSTNAME}"
fi

}

__wezboard_user_vars_preexec() {
  # Tell wezboard the full command that is being run
  __wezboard_set_user_var "WEZBOARD_PROG" "$1"
}
...
if [[ -z "${WEZBOARD_SHELL_SKIP_SEMANTIC_ZONES-}" ]]; then
  if [[ -n "${BLE_VERSION-}" ]]; then
    blehook PRECMD+=__wezboard_semantic_precmd
    blehook PREEXEC+=__wezboard_semantic_preexec
  else
    precmd_functions+=(__wezboard_semantic_precmd)
    preexec_functions+=(__wezboard_semantic_preexec)
  fi
fi

if [[ -z "${WEZBOARD_SHELL_SKIP_USER_VARS-}" ]]; then
  if [[ -n "${BLE_VERSION-}" ]]; then
    blehook PRECMD+=__wezboard_user_vars_precmd
    blehook PREEXEC+=__wezboard_user_vars_preexec
  else
    precmd_functions+=(__wezboard_user_vars_precmd)
    preexec_functions+=(__wezboard_user_vars_preexec)
  fi
fi

if [[ -z "${WEZBOARD_SHELL_SKIP_CWD-}" ]] ; then
  if [[ -n "${BLE_VERSION-}" ]]; then
    blehook PRECMD+=__wezboard_osc7
  else
    precmd_functions+=(__wezboard_osc7)
  fi
fi
