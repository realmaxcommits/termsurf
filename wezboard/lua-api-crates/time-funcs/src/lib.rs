...
/// by the user via `wezboard.time.call_after`.
...
const SCHEDULED_EVENTS: &str = "wezboard-scheduled-events";
...
pub fn register(lua: &Lua) -> anyhow::Result<()> {
    ...
    wezboard_mod.set("sleep_ms", lua.create_async_function(sleep_ms)?)?;
    wezboard_mod.set("strftime", lua.create_function(strftime)?)?;
    wezboard_mod.set("strftime_utc", lua.create_function(strftime_utc)?)?;
    Ok(())
}
