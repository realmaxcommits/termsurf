...
pub fn register(lua: &Lua) -> anyhow::Result<()> {
    ...
    wezboard_mod.set("nerdfonts", NerdFonts {})?;
    wezboard_mod.set("format", lua.create_function(format)?)?;
    wezboard_mod.set(
        "column_width",
    ...
    wezboard_mod.set("permute_any_mods", lua.create_function(permute_any_mods)?)?;
    wezboard_mod.set(
...
pub fn format_as_escapes(items: Vec<FormatItem>) -> anyhow::Result<String> {
    ...
    let mut renderer = new_wezboard_terminfo_renderer();
...
fn permute_mods<'lua>(
    ...
    use wezboard_input_types::Modifiers;
    ...
    static ref CAPS: Capabilities = {
        ...
        Capabilities::new_with_hints(
            ProbeHints::new_from_env()
                ...
                .colorterm(None)
                .colorterm_bce(None)
                .term_program(Some("Wezboard".into()))
                .term_program_version(Some(config::wezboard_version().into())),
        )
        .expect("cannot fail to make internal Capabilities")
    };
}

pub fn new_wezboard_terminfo_renderer() -> TerminfoRenderer {
