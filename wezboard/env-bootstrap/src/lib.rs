...
pub fn set_wezboard_executable() {
    if let Ok(exe) = std::env::current_exe() {
        if let Some(dir) = exe.parent() {
            std::env::set_var("WEZBOARD_EXECUTABLE_DIR", dir);
        }
        std::env::set_var("WEZBOARD_EXECUTABLE", exe);
    }
}
...
pub fn fixup_appimage() {
    if let Some(appimage) = std::env::var_os("APPIMAGE") {
        ...
        // `Wezboard.AppImage foo`, which is super confusing for everyone!
        ...
        // WEZBOARD_EXECUTABLE_DIR is set by `set_wezboard_executable`
        // which is called before `fixup_appimage`
        if let Some(dir) = std::env::var_os("WEZBOARD_EXECUTABLE_DIR") {
        ...
        /// the correct environment when it is launched via wezboard.
        /// However, if we are using the system wezboard to spawn a portable
        /// AppImage then we want these to not take effect.
        fn clean_wezboard_config_env() {
...
pub fn bootstrap() {
    config::assign_version_info(
        wezboard_version::wezboard_version(),
        wezboard_version::wezboard_target_triple(),
    );
    setup_logger();
    register_panic_hook();

    set_wezboard_executable();
    ...
    // wezboard never sets WINDOWID and we don't want to inherit it from a
    // parent process.
    std::env::remove_var("WINDOWID");
    // Avoid vte shell integration kicking in if someone started
    // wezboard or the mux server from inside gnome terminal.
    // <https://github.com/termsurf/termsurf/issues/2237>
