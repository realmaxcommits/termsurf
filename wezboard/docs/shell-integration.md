wezboard supports integrating with the shell through the following means:
...
You can find some [examples for various shells in the wezboard
repo](https://github.com/termsurf/termsurf/tree/main/assets/shell-integration).

To use this file to setup shell integration in wezboard with Bash or Zsh, you can
...
Installing the wezboard shell integration will define the following user vars
by default:

* `WEZBOARD_PROG` - the command line being executed by the shell
* `WEZBOARD_USER` - holds the output from `id -un`; the current user name
* `WEZBOARD_HOST` - holds the output from `hostname`; the hostname that the shell is running on
* `WEZBOARD_IN_TMUX` - holds `1` if the shell is running inside tmux, `0` otherwise
...
The shell integration provides a shell function named `__wezboard_set_user_var` which can be
...
the prompt environment in your `.wezboard.lua`; this example configures
...
configure wezboard to launch clink by setting the `default_prog` configuration
in your `.wezboard.lua`; for example, if you have extracted clink to `c:\clink`
...
if wezboard.target_triple == 'x86_64-pc-windows-msvc' then
