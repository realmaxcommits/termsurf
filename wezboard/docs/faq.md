...
Wezboard will automatically set `LANG` appropriately.
...
and Wezboard will automatically set `LANG` appropriately.
...
emoji, or perhaps some kanji) then wezboard will try to locate a fallback
font that does contain that glyph.

Wezboard uses freetype and harfbuzz to perform font shaping and rendering in a
...
that you need in your `.wezboard.lua`:

```lua
local wezboard = require 'wezboard'

return {
  font = wezboard.font_with_fallback {
    'My Preferred Font',
...
were to emit that same emoji from a script, wezboard would render it correctly.
...
`wezboard` supports [advanced font shaping](config/font-shaping.md), which,
...
experiencing this if, e.g., `!=` becomes rendered as `≠` in `wezboard`.
...
The first thing to note is that `wezboard` will always and only output `UTF-8`
...
section of the docs describes how wezboard processes
...
being set appropriately.  `wezboard` sets this to `xterm-256color` by default,
because wezboard aims to be compatible with with the settings defined by that
...
that wezboard isn't sending the correct input then please [open an
issue](https://github.com/termsurf/termsurf/issues) and include the `xxd` hexdump,
...
Starting in version 20210314-114017-04b7cedd, Wezboard has support for colored
...
You may wish to try these steps to install a copy of a `wezboard` terminfo file;
this will compile a copy of the terminfo and install it into your `~/.terminfo`
directory:

```bash
tempfile=$(mktemp) \
  && curl -o $tempfile https://raw.githubusercontent.com/termsurf/termsurf/master/termwiz/data/wezboard.terminfo \
...
This is not an issue in Wezboard; the same issue manifests in any terminal
...
* Determine the XCursor theme:
  1. is `xcursor_theme` set in the wezboard configuration?
...
4. If the file exists, then wezboard will try to load it

If no XCursor was found, wezboard will fall back to using the default X11 cursor
...
below, and then moving the mouse over the wezboard window:

```
; WEZBOARD_LOG=window::os::x11::cursor=trace wezboard
...
## I'm on macOS and wezboard cannot find things in my PATH

On macOS, wezboard is typically launched directly by the Finder process and inherits
...
However, if you want wezboard to directly spawn some other utility that isn't in that
basic PATH, wezboard will report that it cannot find it.

Probably the easiest to maintain solution is to change something like:

```lua
wezboard.action.SpawnCommandInNewWindow {
  args = { 'nvim', wezboard.config_file },
}
```
...
wezboard.action.SpawnCommandInNewWindow {
  args = {
    os.getenv 'SHELL',
    '-c',
    'nvim ' .. wezboard.shell_quote_arg(wezboard.config_file),
  },
}
```
...
wezboard.action.SpawnCommandInNewWindow {
  args = {
    wezboard.home_dir .. '/.local/bob/nvim-bin/nvim',
...
config.set_environment_variables = {
  ...
  PATH = wezboard.home_dir .. '/.local/bob/nvim-bin:' .. os.getenv 'PATH',
}
```
 ...
 * [wezboard.config_file](config/lua/wezboard/config_file.md)
 * [wezboard.shell_quote_arg](config/lua/wezboard/shell_quote_arg.md)
...
By default, wezboard enables ligature support in the font that you have selected.
