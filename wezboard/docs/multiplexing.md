...
The out-of-the-box experience with `wezboard` allows you to multiplex local tabs
...
Multiplexing in `wezboard` is based around the concept of *multiplexing domains*;
a domain is a distinct set of windows and tabs.  When wezboard starts up it
...
Once connected to a domain, `wezboard` can attach its windows and tabs to the
...
*wezboard also supports [regular ad-hoc ssh connections](ssh.md).
This section of the docs refers to running a wezboard daemon on the remote end
of a multiplexing session that uses ssh as a channel*

A connection to a remote wezboard multiplexer made via an ssh connection is
referred to as an *SSH domain*.  **A compatible version of wezboard must be
...
the wezboard multiplexer daemon on the remote host and connect to
...
```console
$ wezboard connect SSHMUX:my.server
# or to spawn into a new tab in an existing wezboard gui instance:
$ wezboard cli spawn --domain-name SSHMUX:my.server
```

To customize this functionality, see the example for [wezboard.default_ssh_domains()](config/lua/wezboard/default_ssh_domains.md)
...
-- This causes `wezboard` to act as though it was started as
-- `wezboard connect unix` by default, connecting to the unix
-- domain on startup.
...
```console
$ wezboard connect unix
```

Note that in earlier versions of wezboard, a `connect_automatically` domain
...
config.unix_domains = {
  {
    name = 'unix',
    proxy_command = { 'nc', '-U', '/Users/wez/.local/share/termsurf/wezboard/sock' },
  },
}
```
...
between the wezboard client and the server exceeds the specified threshold, the
...
user. This option only applies when `multiplexing = "Wezboard"`.
...
Inside your WSL instance, configure `.wezboard.lua` with this snippet:
...
config.unix_domains = {
  {
    ...
    -- container.
    socket_path = '/mnt/c/Users/USERNAME/.local/share/termsurf/wezboard/sock',
...
config.unix_domains = {
  {
    name = 'wsl',
    serve_command = { 'wsl', 'wezboard-mux-server', '--daemonize' },
  },
}
config.default_gui_startup_args = { 'connect', 'wsl' }
```

Now when you start wezboard you'll be presented with a WSL tab.

You can also omit `default_gui_startup_args` and use:

```console
$ wezboard connect wsl
```
...
Starting with version `20200202-180558-2489abf9`, wezboard can bootstrap a TLS
session by performing an initial connection via SSH to start the wezboard
...
SSH authentication.  Once the connection has been initiated, wezboard
