...
The *cli* subcommand interacts with a running wezboard GUI or multiplexer
...
wezboard uses the following logic to decide which one it should connect to.

* If the `--prefer-mux` flag is passed, then the `wezboard.lua` config file is
  consulted to determine the first *unix domain* defined by the config.
* If the `$WEZBOARD_UNIX_SOCKET` environment variable is set, use that location
...
* If the `$WEZBOARD_PANE` environment variable is set, it will be used
...
See also: [wezboard cli list](list.md)
