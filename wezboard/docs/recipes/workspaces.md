...
to manage separate groups of related tmux windows (which map to tabs in wezboard).

wezboard doesn't have an exact match to tmux sessions, but does have a similar
...
The wezboard GUI is focused on the active workspace, which means that it will
...
When switching the active workspace, wezboard will swap the contents of the
