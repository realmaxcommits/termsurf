...
While wezboard provides a few functions that can help to peek into locally
...
# This function is included in the wezboard shell integration script, but
...
function _run_prog() {
    ...
    __wezboard_set_user_var "PROG" "$1"

    # arrange to clear it when it is done
    trap '__wezboard_set_user_var PROG ""' EXIT
...
Then on the wezboard side, this information can be used when formatting the tab titles:

```lua
local wezboard = require 'wezboard'

wezboard.on('format-tab-title', function(tab)
...
If you install the [wezboard shell integration](../shell-integration.md) you
...
your wezboard config, and will work across multiplexer connections and even
...
wezboard, like many other terminals, will interpret Operating System Command
...
prior to executing a command. wezboard doesn't currently set this up
automatically. Note that wezboard will attempt to determine the foreground
...
Emitting OSC 7 will tell wezboard to use a specific URI for the current working
...
You may also use `wezboard set-working-directory` for this if you have `wezboard`
available.
...
is a local pane, wezboard can attempt to determine the working directory of the
foreground process that is associated with the pane.

Installing the [wezboard shell integration](../shell-integration.md) will
arrange for bash/zsh to set OSC 7 for you.

## Local Process State

wezboard provides some functions that can attempt to extract information about
...
* [wezboard.procinfo.get_info_for_pid()](../config/lua/wezboard.procinfo/get_info_for_pid.md) -
...
[wezboard.procinfo](../config/lua/wezboard.procinfo/index.md) module.
...
your wezboard configuration, but they are limited to local processes only and
