Wezboard provides a searchable scrollback buffer with a configurable maximum
...
You can control whether Wezboard displays a scrollbar via your configuration
file:
...
When the search overlay is active the behavior of wezboard changes:
...
to obtain the defaults and extend them. In earlier versions of wezboard there
...
may be more recent than your version of wezboard) is shown below.

You can see the configuration in your version of wezboard by running
`wezboard show-keys --lua --key-table search_mode`.
...
config.keys = {
  -- search for things that look like git hashes
  {
    key = 'H',
    mods = 'SHIFT|CTRL',
    action = wezboard.action.Search { Regex = '[a-f0-9]{6,}' },
  },
}
```
