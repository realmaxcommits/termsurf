...
Create a file named `.wezboard.lua` in your home directory, with the following
contents:

```lua
-- Pull in the wezboard API
...
-- Finally, return the configuration to wezboard:
return config
```

For more details, see:

- [wezboard.config_builder](lua/wezboard/config_builder.md)
...
`wezboard` will look for a [lua](https://www.lua.org/manual/5.3/manual.html)
configuration file using the logic shown below.

!!! tip
    The recommendation is to place your configuration file at `$HOME/.wezboard.lua`
    (`%USERPROFILE%/.wezboard.lua` on Windows) to get started.

More complex configurations that need to span multiple files can be placed in
`$XDG_CONFIG_HOME/termsurf/termsurf.lua` (for X11/Wayland) or
`$HOME/.config/termsurf/termsurf.lua` (for all other systems).
...
graph TD
  ...
  A -->|No| E{{$WEZBOARD_CONFIG_FILE<br/>environment set?}}
  E -->|Yes| B
  E -->|No| F{{"Running on Windows and<br/>wezboard.lua exists in same<br/>dir as wezboard.exe?<br/>(Thumb drive mode)"}}
  F -->|Yes| B
  F -->|No| H{{Is $XDG_CONFIG_HOME<br/>environment set and<br/>termsurf/termsurf.lua<br/>exists inside it?}}
  H -->|Yes| B
  J --> B
  H -->|No| K{{Does $HOME/.config/termsurf/termsurf.lua exist?}}
  K -->|Yes| B
  K -->|No| J[Use $HOME/.wezboard.lua]
```
{% endraw %}

Prior to version 20210314-114017-04b7cedd, if the candidate file exists but
failed to parse, wezboard would treat it as though it didn't exist and continue
to try other candidate file locations. In all current versions of wezboard, an
error will be shown and the default configuration will be used instead.

!!! note
    On Windows, to support users that carry their wezboard application and
    configuration around on a thumb drive, wezboard will look for the config file in
    the same location as wezboard.exe.  That is shown in the chart above as thumb
    drive mode.  It is **not** recommended to store your configs in that
    location if you are not running off a thumb drive.

`wezboard` will watch the config file that it loads; if/when it changes, the
...
!!! info
    **The configuration file may be evaluated multiple times for each wezboard
    ...
    many copies of wezboard, or are frequently reloading your config file.

### Configuration Overrides

{{since('20210314-114017-04b7cedd')}}

`wezboard` allows overriding configuration values via the command line; here are
a couple of examples:

```bash
$ wezboard --config enable_scroll_bar=true
$ wezboard --config 'exit_behavior="Hold"'
```
...
The `wezboard.lua` configuration file is a lua script which allows for a high
...
* On Windows: a `wezboard_modules` dir in the same directory as `wezboard.exe`. This is for thumb drive mode, and is not recommended to be used otherwise.
* `~/.config/termsurf/wezboard`
* `~/.wezboard`
...
you would place it in `~/.config/termsurf/wezboard/helpers.lua` with contents like this:

```lua
-- I am helpers.lua and I should live in ~/.config/termsurf/wezboard/helpers.lua
