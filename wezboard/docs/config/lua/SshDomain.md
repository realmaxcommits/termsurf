...
config.ssh_domains = {
  {
    ...
    -- The path to the wezboard binary on the remote host.
    -- Primarily useful if it isn't installed in the $PATH
    -- that is configure for ssh.
    -- remote_wezboard_path = "/home/yourusername/bin/wezboard"
  },
}
```
...
* `"Wezboard"` - this is the default; use wezboard's multiplexing client.
  Having wezboard installed on the server is required to use this mode.
* `"None"` - don't use any multiplexing. The connection is an ssh connection
  using the same mechanism as is used by `wezboard ssh`; losing connectivity
...
wezboard to assume that the remote host uses a specific shell command language
dialect, which in turn allows wezboard to respect the
