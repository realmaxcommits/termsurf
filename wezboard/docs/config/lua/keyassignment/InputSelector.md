...
* `choices` - a lua table consisting of the potential choices. Each entry
  ...
  with [wezboard.format](../wezboard/format.md) to produce styled text.
* `action` - and event callback registered via `wezboard.action_callback`.  The
...
config.keys = {
  {
    ...
    action = act.InputSelector {
      action = wezboard.action_callback(function(window, pane, id, label)
        if not id and not label then
          wezboard.log_info 'cancelled'
        else
          wezboard.log_info('you selected ', id, label)
          pane:send_text(id)
        end
      end),
      title = 'I am title',
...
config.keys = {
  {
    ...
    action = wezboard.action_callback(function(window, pane)
      ...
      window:perform_action(
        act.InputSelector {
          action = wezboard.action_callback(
            function(inner_window, inner_pane, id, label)
              if not id and not label then
                wezboard.log_info 'cancelled'
              else
                wezboard.log_info('id = ' .. id)
                wezboard.log_info('label = ' .. label)
