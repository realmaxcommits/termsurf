...
When starting the GUI (not using the `serial` or `connect` subcommands), by default wezboard will set the built-in `"local"` domain as the default multiplexing domain.
...
the default domain to instead use a particular WSL distribution so that wezboard
...
that wezboard can then use [shell integration](../../../shell-integration.md) to
...
then wezboard will by default create a `WslDomain` with the name `"WSL:Ubuntu-18.04"`
...
then when wezboard starts up, it will open with a shell running inside that Ubuntu
