...
If you want to get the most application support out of wezboard, then you may
wish to install a copy of the `wezboard` TERM definition:

```console
$ tempfile=$(mktemp) \
  && curl -o $tempfile https://raw.githubusercontent.com/termsurf/termsurf/main/termwiz/data/wezboard.terminfo \
  && tic -x -o ~/.terminfo $tempfile \
  && rm $tempfile
```

You can then set `term = "wezboard"` in your `.wezboard.lua` config file.
...
you are using has a relatively outdated ncurses installation, the `wezboard`
terminfo will also enable italics and true color support.

If you are using WSL, wezboard will automatically populate `WSLENV` to properly set TERM, COLORTERM, TERM_PROGRAM and TERM_PROGRAM_VERSION in the environment when crossing between win32 and WSL environments.
...
The following snippet works if you installed `wezboard.terminfo` with nix into your user profile. Update the path to `TERMINFO_DIRS` to match the location on your system.
