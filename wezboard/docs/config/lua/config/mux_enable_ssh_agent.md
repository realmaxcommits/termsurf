...
When set to `true` (the default), wezboard will configure the `SSH_AUTH_SOCK`
...
by running `wezboard cli list-clients` and inspecting the `SSH_AUTH_SOCK`
column.

The symlink is updated within (at the time of writing this documentation) 100ms
of the active Mux client changing.

You can set `mux_enable_ssh_agent = false` to prevent wezboard from assigning
