...
Explicitly set the name of the IME server to which wezboard will connect
via the XIM protocol when using X11 and [use_ime](use_ime.md) is `true`.

By default, this option is not set which means that wezboard will consider
...
will cause wezboard to connect to fcitx regardless of the value of `XMODIFIERS`.
