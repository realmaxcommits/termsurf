...
When launching the GUI using either `wezboard` or `wezboard-gui` (with no
subcommand explicitly specified), wezboard will use the value of
`default_gui_startup_args` to pick a default mode for running the GUI.

The default for this config is `{"start"}` which makes `wezboard` with no
additional subcommand arguments equivalent to `wezboard start`.

If you know that you always want to use wezboard's ssh client to login to a
...
which will cause `wezboard` with no additional subcommand arguments to be
equivalent to running `wezboard ssh some-host`.
