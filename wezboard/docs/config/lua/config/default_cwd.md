...
[wezboard.home_dir](../wezboard/home_dir.md) to explicitly refer to your home
directory.

If `wezboard start --cwd /some/path` is used to specify the current working
...
On macOS and Linux, `wezboard` can attempt to resolve the process group leader
...
On Windows, there isn't a process group leader concept, but `wezboard` will
