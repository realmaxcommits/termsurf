...
[wezboard.run_child_process](../wezboard/run_child_process.md)) are not possible
...
-- or `wezboard cli set-tab-title`, but falls back to the
...
* a table holding `FormatItem`s as used in the [wezboard.format](../wezboard/format.md) function.  This allows formatting style and color information for individual elements within the tab.
...
to [tab_max_width](../config/tab_max_width.md).  Wezboard will then compute
...
`wezboard.on("format-tab-title", ...)` calls.
...
wezboard.on(
  ...
  function(tab, tabs, panes, config, hover, max_width)
    ...
    title = wezboard.truncate_right(title, max_width - 2)
