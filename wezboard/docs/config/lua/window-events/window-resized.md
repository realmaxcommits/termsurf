...
live resize operation.  `wezboard` will coalesce the stream of multiple events
...
wezboard.on('window-resized', function(window, pane)
  recompute_padding(window)
end)

wezboard.on('window-config-reloaded', function(window)
  recompute_padding(window)
end)
```
