...
# `wezboard.font(family [, attributes])`
...
return {
  font = wezboard.font 'JetBrains Mono',
}
```
...
oblique) for non-bitmap fonts, wezboard can only select and use fonts that you
...
return {
  font = wezboard.font('JetBrains Mono', { weight = 'Bold' }),
}
```

When resolving fonts from [font_dirs](../config/font_dirs.md), wezboard follows CSS Fonts
...
[wezboard.font_with_fallback](font_with_fallback.md) when you want to specify precise
