---
title: wezboard.config_builder
tags:
 - utility
---

# wezboard.config_builder()
...
local config = {}
if wezboard.config_builder then
  config = wezboard.config_builder()
end
...
When evaluated by earlier versions of wezboard, this config will produce the
...
```
11:44:11.668  WARN   wezboard_dynamic::error > `wrong` is not a valid Config field.  There are too many alternatives to list here; consult the documentation!
```

When using the config builder, the warning message is improved:

```
11:45:23.774  WARN   wezboard_dynamic::error > `wrong` is not a valid Config field.  There are too many alternatives to list here; consult the documentation!
...
The consequence of an error is that wezboard will show a configuration error
