...
status lines and titles in your wezboard configuration.

Wezboard includes `Nerd Font Symbols Font` as a default font fallback
which means that these special symbols are available even without 
requiring you to use a patched font.

This example shows how to lookup `wezboard.nerdfonts.fa_clock_o`, the Font
...
wezboard.on('update-right-status', function(window, pane)
  ...
  window:set_right_status(wezboard.format {
    { Text = wezboard.nerdfonts.fa_clock_o .. ' ' .. date },
  })
end)
```
