...
`wezboard.action` is a special enum constructor type that makes it bit
...
Indexing `wezboard.action` with a valid
...
```
wezboard.action.QuickSelectArgs
```
  ...
  keys = {
    {
      key = ' ',
      mods = 'CTRL|SHIFT',
      action = wezboard.action.QuickSelectArgs,
    },
  },
}
```
