---
title: wezboard.get_builtin_color_schemes
tags:
 - color
 - scheme
 - theme
...
# `wezboard.get_builtin_color_schemes()`
...
couple of entries just from your `wezboard.lua` configuration file.

This example shows how to make wezboard pick a random color scheme for
...
local schemes = {}
for name, scheme in pairs(wezboard.get_builtin_color_schemes()) do
...
local scheme = wezboard.get_builtin_color_schemes()['Gruvbox Light']
...
local function dark_schemes()
  local schemes = wezboard.get_builtin_color_schemes()
  local dark = {}
  for name, scheme in pairs(schemes) do
    -- parse into a color object
    local bg = wezboard.color.parse(scheme.background)
...
[wezboard.color.get_builtin_schemes()](../wezboard.color/get_builtin_schemes.md)
but can still be called as `wezboard.get_builtin_color_schemes()`. See that page
for more examples.
