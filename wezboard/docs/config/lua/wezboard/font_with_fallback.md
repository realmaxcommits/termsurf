---
title: wezboard.font_with_fallback
tags:
 - font
---

# `wezboard.font_with_fallback(families [, attributes])`
...
return {
  font = wezboard.font_with_fallback { 'JetBrains Mono', 'Noto Color Emoji' },
}
```

Wezboard implicitly adds its default fallback to the list that you specify.

The *attributes* parameter behaves the same as that of [wezboard.font](font.md)
...
= `true` will cause wezboard to try to automatically scale using the
