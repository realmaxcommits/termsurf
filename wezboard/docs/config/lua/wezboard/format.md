...
`wezboard.format` can be used to produce a formatted string
with terminal graphic attributes such as bold, italic and colors.
The resultant string is rendered into a string with wezboard
compatible escape sequences embedded.

`wezboard.format` accepts a single array argument, where each
element is a `FormatItem`.

This example logs the text `Hello`, then the date/time, underlined, in purple
text on a blue background to the stderr of the wezboard process:

```lua
local wezboard = require 'wezboard'

local success, date, stderr = wezboard.run_child_process { 'date' }

wezboard.log_info(wezboard.format {
