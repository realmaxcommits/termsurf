# `wezboard.time.call_after(interval_seconds, function)`
...
wezboard.time.call_after(60, function()
  wezboard.reload_configuration()
end)

local amount =
  math.ceil((tonumber(wezboard.time.now():format '%M') / 60) * 255)
