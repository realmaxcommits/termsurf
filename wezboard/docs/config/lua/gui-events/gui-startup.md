...
when running the `wezboard start` subcommand.

It is triggered before any default program is started.

If no explicit program was passed to `wezboard start`, and if the
...
This event does not fire for `wezboard connect` invocations.

{{since('20220807-113146-c2fee766')}}

The event receives an optional [SpawnCommand](../SpawnCommand.md) argument that
corresponds to any arguments that may have been passed via `wezboard start`.
In earlier releases if you implemented this event, you would essentially
prevent `wezboard start -- something` from spawning `something`.
...
local mux = wezboard.mux
local config = {}

wezboard.on('gui-startup', function(cmd)
...
wezboard.on('gui-startup', function(cmd)
  -- allow `wezboard start -- something` to affect what we spawn
  ...
  local project_dir = wezboard.home_dir .. '/wezboard'
...
* [wezboard.mux](../wezboard.mux/index.md)
