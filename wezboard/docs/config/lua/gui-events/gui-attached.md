...
selected domain.  For example, when you use `wezboard connect DOMAIN` or
`wezboard start --domain DOMAIN` to start the GUI, the `gui-attached` event will
...
Note that the `gui-startup` event does not fire when invoking `wezboard connect
DOMAIN` or `wezboard start --domain DOMAIN --attach`.
...
wezboard.on('gui-attached', function(domain)
