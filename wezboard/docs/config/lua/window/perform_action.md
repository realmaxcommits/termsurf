...
The first parameter is a key assignment such as that returned by [`wezboard.action`](../wezboard/action.md).

The second parameter is a `pane` object passed to your event callback.

For an example of this method in action, see [`wezboard.on` Custom Events](../wezboard/on.md#custom-events).
