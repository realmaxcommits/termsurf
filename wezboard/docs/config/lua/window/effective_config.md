...
the `wezboard.lua` file, but represents the fully-populated state of the
...
wezboard.on('show-font-size', function(window, pane)
  wezboard.log_error(window:effective_config().font_size)
end)
  ...
  keys = {
    {
      key = 'E',
      mods = 'CTRL',
      action = wezboard.action.EmitEvent 'show-font-size',
    },
  },
}
```
