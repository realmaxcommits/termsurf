...
**NOTE: You probably want to use [wezboard.gui.get_appearance()](../wezboard.gui/get_appearance.md) instead, as it is easier to use!**
...
wezboard is able to detect when the appearance has changed and will generate a
...
appearance, as wezboard didn't know how to interrogate the appearance on Wayland
...
function query_appearance_gnome()
  local success, stdout = wezboard.run_child_process {
    'gsettings',
    'get',
...
Since Wezboard will not fire a `window-config-reloaded` event on Wayland for
older versions of wezboard, you will instead need to listen on the
