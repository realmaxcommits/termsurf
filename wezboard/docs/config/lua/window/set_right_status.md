...
It is recommended that you use [wezboard.format](../wezboard/format.md) to compose
the string.

Here's a basic example that displays the time in the status area:

![Demonstrating setting the right status area to the current date and time](../../../screenshots/wezboard-status-date.png)

```lua
local wezboard = require 'wezboard'

wezboard.on('update-right-status', function(window, pane)
  local date = wezboard.strftime '%Y-%m-%d %H:%M:%S'

  -- Make it italic and underlined
  window:set_right_status(wezboard.format {
...
![Demonstrating setting the right status area with powerline styling](../../../screenshots/wezboard-status-powerline.png)
...
wezboard.on('update-right-status', function(window, pane)
  ...
  if cwd_uri then
    ...
    if type(cwd_uri) == 'userdata' then
      -- Running on a newer version of wezboard and we have
      -- a URL object here, making this simple!

      cwd = cwd_uri.file_path
      hostname = cwd_uri.host or wezboard.hostname()
    else
      -- an older version of wezboard, 20230712-072601-f4abf8fd or earlier,
  ...
  local date = wezboard.strftime '%a %b %-d %H:%M'
  table.insert(cells, date)

  -- An entry for each battery (typically 0 or 1 battery)
  for _, b in ipairs(wezboard.battery_info()) do
  ...
  window:set_right_status(wezboard.format(elements))
end)
```
