...
wezboard.on('toggle-ligature', function(window, pane)
  ...
  keys = {
    {
      key = 'E',
      mods = 'CTRL',
      action = wezboard.action.EmitEvent 'toggle-ligature',
    },
  },
}
```
...
wezboard.on('toggle-opacity', function(window, pane)
  local overrides = window:get_config_overrides() or {}
  ...
  keys = {
    {
      key = 'B',
      mods = 'CTRL',
      action = wezboard.action.EmitEvent 'toggle-opacity',
    },
  },
}
```
