...
```lua
local tab, pane, window = wezboard.mux.spawn_window {}
```
...
```lua
wezboard.mux.spawn_window { args = { 'top' } }
```
...
```lua
wezboard.mux.spawn_window { cwd = '/tmp' }
```
...
```lua
wezboard.mux.spawn_window { set_environment_variables = { FOO = 'BAR' } }
```
...
```lua
wezboard.mux.spawn_window { domain = { DomainName = 'my.name' } }
```
...
```lua
wezboard.mux.spawn_window { width = 60, height = 30 }
```
...
```lua
wezboard.mux.spawn_window { workspace = { 'coding' } }
```
...
wezboard.mux.spawn_window {
  position = {
    ...
    -- * {Named="HDMI-1"} - uses a screen by name. See wezboard.gui.screens()
    -- origin = "ScreenCoordinateSystem"
  },
}
```

See also [wezboard.gui.screens()](../wezboard.gui/screens.md)
