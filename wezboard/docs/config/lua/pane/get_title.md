...
Returns the title of the pane.  This will typically be `wezboard` by default but
...
If the title text is `wezboard` and the pane is a local pane, then wezboard will
attempt to resolve the executable path of the foreground process that is
associated with the pane and will use that instead of `wezboard`.
