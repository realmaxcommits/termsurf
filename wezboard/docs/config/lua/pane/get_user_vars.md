...
also recognized by wezboard; this example sets the `foo` user variable
to the value `bar`:
...
__wezboard_set_user_var "foo" "bar"
```

you're then able to access this in your wezboard config:

```lua
wezboard.log_info('foo var is ' .. pane:get_user_vars().foo)
```
