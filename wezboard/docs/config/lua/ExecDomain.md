...
wezboard to explicitly spawn a command that runs `docker exec`, you would also
...
You must use the `wezboard.exec_domain` function to define a domain. It accepts
the following parameters:

```
wezboard.exec_domain(NAME, FIXUP [, LABEL])
```
...
```lua
wezboard.exec_domain('myname', function(cmd)
  return cmd
end)
```
...
what wezboard will execute in order to satisfy the user's request to
spawn a new program.
...
-- wezboard.exec_domain()
wezboard.exec_domains(domain_name, fixup_func, function(domain_name)
  return domain_name
end)
```
...
[wezboard.format()](wezboard/format.md) to manage that.
...
config.exec_domains = {
  ...
  -- killed.
  wezboard.exec_domain('scoped', function(cmd)
    -- The "cmd" parameter is a SpawnCommand object.
    -- You can log it to see what's inside:
    wezboard.log_info(cmd)

    -- Synthesize a human understandable scope name that is
    -- (reasonably) unique. WEZBOARD_PANE is the pane id that
    -- will be used for the newly spawned pane.
    -- WEZBOARD_UNIX_SOCKET is associated with the wezboard
    -- process id.
    local env = cmd.set_environment_variables
    local ident = 'wezboard-pane-'
      .. env.WEZBOARD_PANE
      .. '-on-'
      .. basename(env.WEZBOARD_UNIX_SOCKET)
    ...
    local wrapped = {
      ...
      '--user',
      '--scope',
      '--description=Shell started by wezboard',
      '--same-dir',
      '--collect',
      '--unit=' .. ident,
    }
...
-- spawned by wezboard will have its own scope
...
function docker_list()
  local docker_list = {}
  local success, stdout, stderr = wezboard.run_child_process {
    'docker',
    'container',
    'ls',
    '--format',
  ...
  for _, line in ipairs(wezboard.split_by_newlines(stdout)) do
