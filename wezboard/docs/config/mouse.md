...
*Note: you can run `wezboard show-keys` to show the effective key and mouse assignments*.
...
NOTE: In the action column, `act` is an alias to `wezboard.action` (to avoid repetition):
