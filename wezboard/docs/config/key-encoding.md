...
The default encoding scheme used by wezboard is xterm compatible, but there are
...
When wezboard receives the sequence `CSI >4;Nm`, where `N` is `0`, `1` or `2`,
...
wezboard will honor the application requests to modify the keyboard encoding.

## Windows

On Windows, [allow_win32_input_mode](lua/config/allow_win32_input_mode.md)
defaults to `true` which causes wezboard to listen for an escape sequence
