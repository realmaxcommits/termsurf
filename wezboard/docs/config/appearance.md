...
Wezboard ships with over 700 color schemes available from
...
[wezboard.gui.get_appearance()](lua/wezboard.gui/get_appearance.md)
...
you need to use [wezboard.color.get_default_colors()](lua/wezboard.color/get_default_colors.md) and explicitly merge them.
...
### Defining a Color Scheme in your `.wezboard.lua`
...
Color schemes names that you define in your `wezboard.lua` take precedence
...
See also [wezboard.get_builtin_color_schemes()](lua/wezboard/get_builtin_color_schemes.md) for
...
the available color schemes for an example](https://github.com/termsurf/termsurf/tree/main/config/src/scheme_data.rs).

It is recommended that you place your custom scheme in a directory
named `$HOME/.config/termsurf/wezboard/colors` if you're on a POSIX system.

On a Windows system, `wezboard` will search for schemes in a directory
named `colors` that is in the same directory as the `wezboard.exe`.

If you wish to place your color scheme files in some other location, then you
will need to instruct wezboard where to look for your scheme files; the
...
Wezboard supports dynamically changing its color palette via escape sequences.
...
```
  ...
  <video width="80%" controls src="../screenshots/wezboard-dynamic-colors.mp4" loop></video>
...
config.window_frame = {
  ...
  font = wezboard.font { family = 'Roboto', weight = 'Bold' },
...
![Screenshot](../screenshots/wezboard-vday-screenshot.png)

{{since('20201031-154415-9614e117')}}

You can attach an image to the background of the wezboard window:
...
to the directory containing your `wezboard.lua` config file.
...
If your Operating System provides Compositing support then Wezboard is able to
