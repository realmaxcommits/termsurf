...
`keys` section in your `~/.wezboard.lua` config file.  For example, you can
 ...
 * `LEADER` - a special modal modifier state managed by `wezboard`. See [Leader Key](#leader-key) for more information.
...
If you omit an explicit prefix, wezboard will assume `phys:` and use the
...
In some cases, `wezboard` may not know how to represent a key event in either
...
config.keys = {
  ...
  -- Send "CTRL-A" to the terminal when pressing CTRL-A, CTRL-A
  {
    key = 'a',
    mods = 'LEADER|CTRL',
    action = wezboard.action.SendKey { key = 'a', mods = 'CTRL' },
  },
}
```
...
config.keys = {
  ...
  },
  {
    key = '-',
    mods = 'LEADER',
    action = wezboard.action.SplitVertical { domain = 'CurrentPaneDomain' },
  },
}
```
