...
`ci/deploy.sh` is a script that is used to build packages in wezboard's CI.
...
Wezboards version number is derived from the date of the commit from which it
was released:
...
If you are not building wezboard from its git repo, wezboard will read a file named
`.tag` in the root of its source tree to determine the version.

Please package using the `wezboard-YYYYMMDD-HHMMSS-HASH-src.tar.gz` release
...
If you must decorate wezboard's version, then it is recommend that you append
your supplemental extra version information on the end of the wezboard's
version.
...
* `wezboard-mux-server` - multiplexer server. No gui required. Likely want this
  in a separate package from `wezboard-gui` so it can run on a headless system.
* `wezboard-gui` - the GUI portion of the terminal
* `wezboard` - the CLI and frontend that knows how to launch the GUI. It is
...
* `assets/shell-integration`, `assets/shell-completion`: should be deployed along with the `wezboard` executable
* `assets/wezboard.desktop`, `assets/wezboard.appdata.xml`, `assets/wezboard-nautilus.py`: should be deployed along with `wezboard-gui`

## Building wezboard

It is recommended that you enable the `distro-defaults` rust feature
when building for a distro (`cargo build --release -p wezboard-gui --features distro-defaults`).
...
By default, wezboard will compile in a handful of fonts in order to provide a
...
Note that wezboard requires at least the following fonts to be available, either
...
to maintain your wezboard package.

### Building without wayland support

If your distro doesn't include any support for Wayland, you will need to
disable that feature when you build wezboard:

```
cargo build --release -p wezboard-gui --no-default-features --features distro-defaults,vendored-fonts
```
