...
fn derive_struct(input: &DeriveInput, fields: &FieldsNamed) -> Result<TokenStream> {
    ...
    let bound = parse_quote!(wezboard_dynamic::FromDynamic);
    ...
    let from_dynamic = match info.try_from {
        Some(try_from) => {
            quote!(
                ...
                <#ident>::try_from(target).map_err(|e| wezboard_dynamic::Error::Message(format!("{:#}", e)))
            )
        }
        None => {
            quote!(
                match value {
                    Value::Object(obj) => {
                        wezboard_dynamic::Error::raise_unknown_fields(options, #literal, &obj, Self::possible_field_names())?;
                        #obj
                    }
                    other => Err(wezboard_dynamic::Error::NoConversion {
    ...
    let tokens = quote! {
        impl #impl_generics wezboard_dynamic::FromDynamic for #ident #ty_generics #bounded_where_clause {
            fn from_dynamic(value: &wezboard_dynamic::Value, options: wezboard_dynamic::FromDynamicOptions) -> core::result::Result<Self, wezboard_dynamic::Error> {
                use wezboard_dynamic::{Value, BorrowedKey, ObjectKeyTrait};
                #adjust_options
                #from_dynamic
            }
...
fn derive_enum(input: &DeriveInput, enumeration: &DataEnum) -> Result<TokenStream> {
                        ...
                        quote!(
                            #literal => {
                                match value {
                                    ...
                                    other => return Err(wezboard_dynamic::Error::NoConversion {
                                        source_type: other.variant_name().to_string(),
                                        dest_type: "Object",
                                    }),
                                }
                            ...
                            let var_fields = fields
                                ...
                                .map(|(idx, f)| {
                                    ...
                                    quote!(
                                        <#ty>::from_dynamic(
                                            arr.get(#idx)
                                            .ok_or_else(|| wezboard_dynamic::Error::Message(
            ...
            quote!(
                    match value {
                        Value::String(s) => {
                            match s.as_str() {
                                #( #units )*
                                _ => Err(wezboard_dynamic::Error::InvalidVariantForType {
                        ...
                        Value::Object(place) => {
                            if place.len() == 1 {
                                ...
                                Err(wezboard_dynamic::Error::IncorrectNumberOfEnumKeys {
                                    type_name: #literal,
                                    num_keys: place.len(),
                                })
                            }
    ...
    let tokens = quote! {
        impl wezboard_dynamic::FromDynamic for #ident {
            fn from_dynamic(value: &wezboard_dynamic::Value, options: wezboard_dynamic::FromDynamicOptions) -> core::result::Result<Self, wezboard_dynamic::Error> {
                use wezboard_dynamic::{Value, BorrowedKey, ObjectKeyTrait};
                #from_dynamic
            }
