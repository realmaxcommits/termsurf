...
use std::path::Path;
use std::sync::Mutex;
use wezboard_dynamic::{
    FromDynamic, FromDynamicOptions, ToDynamic, UnknownFieldAction, Value as DynValue,
};

pub use mlua;

static LUA_REGISTRY_USER_CALLBACK_COUNT: &str = "wezboard-user-callback-count";
...
pub fn get_or_create_sub_module<'lua>(
    ...
    let sub = wezboard_mod.get(name)?;
    match sub {
        Value::Nil => {
            let sub = lua.create_table()?;
            wezboard_mod.set(name, sub.clone())?;
            Ok(sub)
        }
        Value::Table(sub) => Ok(sub),
        wat => anyhow::bail!(
            "cannot register module wezboard.{name} as it is already set to a value of type {}",
            wat.type_name()
        ),
    }
}
        ...
        DynValue::Object(obj) => {
            match obj.get_by_str(&key) {
                None => {
                    ...
                    wezboard_dynamic::Error::warn(message);
                }
                Some(_dvalue) => {
...
/// wezboard specific config paths for lua modules, should
/// they choose to `require` additional code from their config.
///
/// A `wezboard` module is registered so that the script can
/// `require "wezboard"` and call into functions provided by
/// wezboard.  The wezboard module contains:
/// * `executable_dir` - the directory containing the wezboard
///   executable.  This is potentially useful for portable
///   installs on Windows.
/// * `config_dir` - the directory containing the wezboard
///   configuration.
/// * `log_error` - a function that logs to stderr (or the server
///   log file for daemonized wezboard).
/// * `target_triple` - the rust compilation target triple.
/// * `version` - the version of the running wezboard instance.
...
pub fn make_lua_context(config_file: &Path) -> anyhow::Result<Lua> {
    ...
    {
        let globals = lua.globals();
        // This table will be the `wezboard` module in the script
        let wezboard_mod = get_or_create_module(&lua, "wezboard")?;
        ...
        prefix_path(&mut path_array, &crate::HOME_DIR.join(".wezboard"));
        ...
        if let Ok(exe) = std::env::current_exe() {
            if let Some(path) = exe.parent() {
                wezboard_mod
                    ...
                    .context("set wezboard.executable_dir")?;
                if cfg!(windows) {
                    // For a portable windows install, force in this path ahead
                    // of the rest
                    prefix_path(&mut path_array, &path.join("wezboard_modules"));
                }
            }
        }
...
package.searchers[2] = function(module)
  ...
  if name then
    package.loaded.wezboard.add_to_config_reload_watch_list(name)
  end
  return orig(module)
end
        "#,
        ...
        wezboard_mod
            ...
            .context("set wezboard.config_file")?;
        wezboard_mod
            .set(
                "config_dir",
                config_dir
            ...
            .context("set wezboard.config_dir")?;

        lua.set_named_registry_value("wezboard-watch-paths", Vec::<String>::new())?;
        wezboard_mod.set(
            "add_to_config_reload_watch_list",
            lua.create_function(add_to_config_reload_watch_list)?,
        )?;

        wezboard_mod.set("target_triple", crate::wezboard_target_triple())?;
        wezboard_mod.set("version", crate::wezboard_version())?;
        wezboard_mod.set("home_dir", crate::HOME_DIR.to_str())?;
        wezboard_mod.set(
            "running_under_wsl",
        ...
        wezboard_mod.set("font", lua.create_function(font)?)?;
        wezboard_mod.set(
            "font_with_fallback",
            lua.create_function(font_with_fallback)?,
        )?;
        wezboard_mod.set("hostname", lua.create_function(hostname)?)?;
        wezboard_mod.set("action", luahelper::enumctor::Enum::<KeyAssignment>::new())?;
        wezboard_mod.set(
            "has_action",
        ...
        wezboard_mod.set("action_callback", lua.create_function(action_callback)?)?;
        wezboard_mod.set("exec_domain", lua.create_function(exec_domain)?)?;

        wezboard_mod.set("utf16_to_utf8", lua.create_function(utf16_to_utf8)?)?;
        wezboard_mod.set("split_by_newlines", lua.create_function(split_by_newlines)?)?;
        wezboard_mod.set("on", lua.create_function(register_event)?)?;
        wezboard_mod.set("emit", lua.create_async_function(emit_event)?)?;
        wezboard_mod.set("shell_join_args", lua.create_function(shell_join_args)?)?;
        wezboard_mod.set("shell_quote_arg", lua.create_function(shell_quote_arg)?)?;
        wezboard_mod.set("shell_split", lua.create_function(shell_split)?)?;
        ...
        // those values since wezboard was started
...
/// <https://github.com/termsurf/termsurf/issues/1736>
/// <https://github.com/termsurf/termsurf/issues/1786>
...
///
/// `wezboard.font("foo", {foreground="tomato"})`
/// yields:
...
///
/// `wezboard.font_with_fallback({"Operator Mono", "DengXian"})`
///
/// The second optional argument is a list of other TextStyle fields,
/// as described by the `wezboard.font` documentation.
...
/// This implements `wezboard.on`, whose goal is to register an event handler
/// callback.
...
///
/// ```lua
/// wezboard.on("event-name", function(arg1, arg2)
///   -- do something
///   return false -- if you want to prevent other handlers running
/// end);
///
/// wezboard.emit("event-name", "foo", "bar");
/// ```
pub fn register_event<'lua>(
    lua: &'lua Lua,
    (name, func): (String, mlua::Function),
) -> mlua::Result<()> {
    let decorated_name = format!("wezboard-event-{}", name);
...
const IS_EVENT: &str = "wezboard-is-event-emission";
...
pub fn add_to_config_reload_watch_list<'lua>(
    ...
    let mut watch_paths: Vec<String> = lua.named_registry_value("wezboard-watch-paths")?;
    watch_paths.extend_from_slice(&args);
    lua.set_named_registry_value("wezboard-watch-paths", watch_paths)?;
    Ok(())
}
...
assert(wezboard.emit('foo', 2) == false)

wezboard.on('bar', function (n, str)
...
assert(wezboard.emit('bar', 42, 'woot') == true)
"#,
            )
            .exec_async(),
        )?;
