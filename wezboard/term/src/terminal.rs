...
use std::sync::Arc;
use wezboard_escape_parser::parser::Parser;
...
pub trait DeviceControlHandler: Send + Sync {
    fn handle_device_control(&mut self, _control: wezboard_escape_parser::DeviceControlMode);
}
...
impl Terminal {
    ...
    pub fn perform_actions(&mut self, actions: Vec<wezboard_escape_parser::Action>) {
        self.state.increment_seqno();
        {
            let mut performer = Performer::new(&mut self.state);
