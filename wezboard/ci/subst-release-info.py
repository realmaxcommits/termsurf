...
CATEGORIZE = {
    ...
    r"^wezboard-\d+-\d+-[a-f0-9]+.tar.xz$": "linux_raw_bin",
    r"src.tar.gz$": "src",
    r"^Wezboard-macos-.*.zip$": "macos_zip",
    r"^Wezboard-windows-.*.zip$": "windows_zip",
    r"^Wezboard-.*.setup.exe$": "windows_exe",
...
def categorize(rel):
    ...
    tag_name = "wezboard-%s" % rel["tag_name"]
...
def load_release_info():
    with open("/tmp/wezboard.releases.json") as f:
        release_info = json.load(f)

    with open("/tmp/wezboard.nightly.json") as f:
        nightly = json.load(f)
