...
URL="https://github.com/termsurf/termsurf/releases/download/${TAG_NAME}/wezboard-${TAG_NAME}-src.tar.gz"
...
SHA256=$(sha256sum wezboard*-src.tar.gz | cut -d' ' -f1)

sed -e "s,@URL@,$URL,g" -e "s/@SHA256@/$SHA256/g" < assets/flatpak/com.termsurf.wezboard.template.json > flathub/com.termsurf.wezboard.json

RELEASE_DATE=$(git -c "core.abbrev=8" show -s "--format=%cd" "--date=format:%Y-%m-%d")
sed -e "s,@TAG_NAME@,$TAG_NAME,g" -e "s/@DATE@/$RELEASE_DATE/g" < assets/flatpak/com.termsurf.wezboard.appdata.template.xml > flathub/com.termsurf.wezboard.appdata.xml

cd flathub
git config user.email wezboard@termsurf.com
git config user.name 'Wez Longboard'
