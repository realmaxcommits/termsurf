...
[[ -f /tmp/wezboard.releases.json ]] || ghapi /repos/termsurf/termsurf/releases > /tmp/wezboard.releases.json
[[ -f /tmp/wezboard.nightly.json ]] || ghapi /repos/termsurf/termsurf/releases/tags/nightly > /tmp/wezboard.nightly.json
...
cp "assets/icon/wezboard-icon.svg" docs/favicon.svg
mkdir -p docs/fonts
...
docker_or_podman build -t wezboard/mkdocs-material -f ci/Dockerfile.docs .

if [ "$SERVE" == "yes" ] ; then
  docker_or_podman run --rm -it -p8000:8000 -v ${PWD}:/docs wezboard/mkdocs-material serve -a 0.0.0.0:8000
  #docker_or_podman run --rm -it --network=host -v ${PWD}:/docs wezboard/mkdocs-material $@
else
  docker_or_podman run --rm -e CARDS=true -v ${PWD}:/docs wezboard/mkdocs-material build
fi
