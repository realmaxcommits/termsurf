...
""",
    ...
    def upload_artifact(self):
        ...
        if self.uses_yum():
            ...
            steps += [
                ...
                RunStep(
                    "Rename APKs",
                    f"mv ~/packages/wezboard/x86_64/*.apk $(echo ~/packages/wezboard/x86_64/*.apk | sed -e 's/wezboard-/wezboard-{self.name}-/')",
                ),
                # Move it to the repo dir
                RunStep(
                    "Move APKs",
                    f"mv ~/packages/wezboard/x86_64/*.apk .",
                ),
                # Move and rename the keys
                RunStep(
                    "Move APK keys",
                    f"mv ~/.abuild/*.pub wezboard-{self.name}.pub",
                ),
            ]
        elif self.uses_zypper():
            steps.append(
    ...
    def asset_patterns(self):
        ...
        if self.uses_yum() or self.uses_zypper():
            patterns += ["wezboard-*.rpm"]
        elif "win" in self.name:
            patterns += ["Wezboard-*.zip", "Wezboard-*.exe"]
        elif "mac" in self.name:
            patterns += ["Wezboard-*.zip"]
        elif ("ubuntu" in self.name) or ("debian" in self.name):
            patterns += ["wezboard-*.deb", "wezboard-*.xz"]
        elif "alpine" in self.name:
            patterns += ["wezboard-*.apk"]
            if self.is_tag:
                patterns.append("*.pub")
    ...
    def upload_artifact_nightly(self):
        ...
        if self.uses_yum() or self.uses_zypper():
            ...
            script = ""
            # Note that 'wezboard' MUST be last in this list,
            # otherwise the globbing will mess things up
            for pkg in ['wezboard-common', 'wezboard-gui', 'wezboard-mux-server', 'wezboard']:
            ...
            steps.append(
                RunStep(
                    "Move APKs",
                    f"mv ~/packages/wezboard/x86_64/*.apk wezboard-nightly-{self.name}.apk",
                )
            )
    ...
    def upload_asset_nightly(self):
        ...
        if self.container == "ubuntu:22.04":
            steps += [
                RunStep(
                    "Upload to gemfury",
                    f"for f in wezboard*.deb ; do curl -i -F package=@$f https://$FURY_TOKEN@push.fury.io/wez/ ; done",
    ...
    def create_flathub_pr(self):
        ...
        return [
            ActionStep(
                "Checkout flathub/com.termsurf.wezboard",
                action="actions/checkout@v4",
                params={
                    "repository": "flathub/com.termsurf.wezboard",
                    "path": "flathub",
            ...
            RunStep(
                "Submit PR",
                'cd flathub && gh pr create --fill --body "PR automatically created by release automation in the wezboard repo"',
                env={
        ...
        if "windows" in self.name:
            steps += [
                ...
                RunStep(
                    ...
                    "cd winget-pkgs && git config user.email wezboard@termsurf.com",
                ),
                RunStep(
                    "Setup name for winget repo",
                    "cd winget-pkgs && git config user.name 'Wez Longboard'",
                ),
                RunStep(
                    "Create winget manifest and push to fork",
                    "bash ci/make-winget-pr.sh winget-pkgs Wezboard-*.exe",
                ),
                RunStep(
                    "Submit PR",
                    'cd winget-pkgs && gh pr create --fill --body "PR automatically created by release automation in the wezboard repo"',
                    env={
                        "GITHUB_TOKEN": "${{ secrets.GH_PAT }}",
                    },
                ),
        ...
        if "macos" in self.name:
            steps += [
                ActionStep(
                    ...
                    action="actions/checkout@v4",
                    params={
                        "repository": "wez/homebrew-wezboard",
                        "path": "homebrew-wezboard",
                        "token": "${{ secrets.GH_PAT }}",
                    },
                ),
                RunStep(
                    "Update homebrew tap formula",
                    "cp wezboard.rb homebrew-wezboard/Casks/wezboard.rb",
                ),
                ActionStep(
            ...
            steps += [
                ActionStep(
                    ...
                    action="actions/checkout@v4",
                    params={
                        "repository": "wez/homebrew-wezboard-linuxbrew",
                        "path": "linuxbrew-wezboard",
                        "token": "${{ secrets.GH_PAT }}",
                    },
                ),
                RunStep(
                    "Update linuxbrew tap formula",
                    "cp wezboard-linuxbrew.rb linuxbrew-wezboard/Formula/wezboard.rb",
                ),
                ActionStep(
        ...
        if self.container:
            steps += [
                RunStep(
                    ...
                    "git config --global --add safe.directory /__w/termsurf/termsurf",
                )
            ]
...
"""
  ...
  upload:
    ...
    needs: build
    if: github.repository == 'termsurf/termsurf'
    permissions:
      contents: write
      pages: write
      id-token: write
