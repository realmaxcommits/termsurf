...
#flatpak run --env=G_DEBUG=fatal-criticals org.freedesktop.appstream-glib validate assets/wezboard.appdata.xml
...
if [ "${CI}" != "yes" ] ; then
  flatpak-builder \
    --state-dir /var/tmp/wezboard-flatpak-builder \
    --install /var/tmp/wezboard-flatpak-repo \
    assets/flatpak/com.termsurf.wezboard.json \
    --force-clean --user -y
fi
