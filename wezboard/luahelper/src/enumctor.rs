...
/// The primary usage is to enable `wezboard.action` to have the following
/// behaviors for KeyAssignment:
///
/// `wezboard.action{QuickSelectArgs={}}` -> compatibility with prior versions;
/// the table is passed through and from_dynamic -> lua conversion is attempted.
///
/// `wezboard.action.QuickSelectArgs` -> since the `QuickSelectArgs` variant
/// has a payload that impl Default, this is equivalent to the call above.
///
/// `wezboard.action.QuickSelectArgs{}` -> equivalent to the call above, but
/// explicitly calls the constructor with no parameters.
///
/// `wezboard.action.QuickSelectArgs{alphabet="abc"}` -> configures the alphabet.
///
