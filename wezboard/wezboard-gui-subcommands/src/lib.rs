...
pub const DEFAULT_WINDOW_CLASS: &str = "com.termsurf.wezboard";
...
pub struct StartCommand {
    ...
    /// in your wezboard configuration file.
    #[arg(long = "no-auto-connect")]
    pub no_auto_connect: bool,

    /// If enabled, don't try to ask an existing wezboard GUI instance
    /// to start the command.  Instead, always start the GUI in this
    /// invocation of wezboard so that you can wait for the command
    /// to complete by waiting for this wezboard process to finish.
    ...
    /// The default is "com.termsurf.wezboard".
    ...
    /// instance of wezboard, including error, update and ssh
    ...
    /// wezboard will simply attach and will NOT spawn the specified PROG.
    /// If you omit --attach when using --domain, wezboard will attach
    /// AND then spawn PROG.
    #[arg(long, requires = "domain")]
    pub attach: bool,

    /// Instead of executing your shell, run PROG.
    /// For example: `wezboard start -- bash -l` will spawn bash
...
pub struct SshCommand {
    ...
    /// `wezboard ssh` is able to parse some (but not all!) options
    ...
    /// For example:
    ///
    /// `wezboard ssh -oIdentityFile=/secret/id_ed25519 some-host`
    #[arg(
        long = "ssh-option",
        short = 'o',
        name = "name=value",
    ...
    /// Instead of executing your shell, run PROG.
    /// For example: `wezboard ssh user@host -- bash -l` will spawn bash
