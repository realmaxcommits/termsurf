...
impl Header {
    fn new(config: &ConfigHandle, size: PtySize, prog: &[&OsStr]) -> Self {
        ...
        env.insert(
            "WEZBOARD_VERSION".to_string(),
            config::wezboard_version().to_string(),
        );
        env.insert(
            "WEZBOARD_TARGET_TRIPLE".to_string(),
            config::wezboard_target_triple().to_string(),
        );
...
pub struct RecordCommand {
    ...
    /// the default_cwd defined by your wezboard configuration
    #[arg(long)]
