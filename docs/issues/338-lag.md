...
- [ ] What causes the large gaps? → **Unknown, needs investigation**

#### Phase 2: Investigate CEF Frame Production

**Status:** IN PROGRESS

The bottleneck is confirmed to be CEF. Next steps:

1. Check if scroll events are reaching CEF at 60Hz
2. Investigate CEF message loop (`do_message_loop_work()` + 1ms sleep)
3. Check if `windowless_frame_rate: 60` is being respected
4. Look for CEF internal throttling or vsync behavior

#### Phase 3: Fix Based on Findings

Depends on Phase 2 findings:

- If CEF message loop is slow → Try removing 1ms sleep, use adaptive polling
- If CEF is throttling internally → Check CEF settings, try different
  `windowless_frame_rate` values
- If scroll events are delayed → Fix input path
