...
**Goal:** Use WezTerm's existing `inactive_pane_hsb` config and HSB infrastructure
for all webview-related dimming.
...
The control panel uses `render_element()` which calls `quad.set_hsv(None)`.
The webview uses a separate shader with a simple `dim_factor` uniform.
...
1. [ ] Webview uses `inactive_pane_hsb` config values
