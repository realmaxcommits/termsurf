...
def assert_inventory_split() -> None:
    ...
    require(
        "quick-terminal visuals" in runtime_inventory,
        "remaining macOS GUI gap omitted quick terminal evidence",
    )
    require(
        "split visual/layout parity" in runtime_inventory,
        "remaining macOS GUI gap omitted split visual evidence",
    )
    require("72 rows Oracle complete" in config_matrix, "CFG-223 oracle count not updated")
    require("75 rows closed" in config_matrix, "CFG-223 closed count not updated")
