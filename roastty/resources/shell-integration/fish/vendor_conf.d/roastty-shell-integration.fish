...
function roastty_restore_xdg_data_dir -d "restore the original XDG_DATA_DIR value"
    # If we don't have our own data dir then we don't need to do anything.
    if not set -q ROASTTY_SHELL_INTEGRATION_XDG_DIR
        return
    end
    ...
    if set --function index (contains --index "$ROASTTY_SHELL_INTEGRATION_XDG_DIR" $xdg_data_dirs)
    ...
    set --erase ROASTTY_SHELL_INTEGRATION_XDG_DIR
end

function roastty_exit -d "exit the shell integration setup"
    functions -e roastty_restore_xdg_data_dir
    functions -e roastty_exit
    exit 0
end

# We always try to restore the XDG data dir
roastty_restore_xdg_data_dir

# If we aren't interactive or we've already run, don't run.
status --is-interactive || roastty_exit
...
function __roastty_setup --on-event fish_prompt -d "Setup roastty integration"
    functions -e __roastty_setup

    set --local features (string split , $ROASTTY_SHELL_FEATURES)
    ...
    set -g __roastty_prompt_start_mark "\e]133;A\a"
    if test "$fish_major" -gt 4; or test "$fish_major" -eq 4 -a "$fish_minor" -ge 1
        set -g __roastty_prompt_start_mark "\e]133;A;click_events=1\a"
    end
    ...
    if string match -q 'cursor*' -- $features
        ...
        function __roastty_set_cursor_beam --on-event fish_prompt -V cursor -d "Set cursor shape"
            if not functions -q fish_vi_cursor_handle
                echo -en "\e[$cursor q"
            end
        end
        function __roastty_reset_cursor --on-event fish_preexec -d "Reset cursor shape"
            if not functions -q fish_vi_cursor_handle
                echo -en "\e[0 q"
            end
        end
    ...
    if contains path $features; and test -n "$ROASTTY_BIN_DIR"
        fish_add_path --global --path --append "$ROASTTY_BIN_DIR"
    end
    ...
    set -l features (string split ',' -- "$ROASTTY_SHELL_FEATURES")
    if contains ssh-env $features; or contains ssh-terminfo $features
        function ssh --wraps=ssh --description "SSH wrapper with Roastty integration"
            set -l features (string split ',' -- "$ROASTTY_SHELL_FEATURES")
            set -l flags
            contains ssh-env $features; or set -a flags --forward-env=false
            contains ssh-terminfo $features; or set -a flags --terminfo=false
            "$ROASTTY_BIN_DIR/roastty" +ssh $flags -- $argv
        end
    end

    # Setup prompt marking
    function __roastty_mark_prompt_start --on-event fish_prompt --on-event fish_posterror
        # If we never got the output end event, then we need to send it now.
        if test "$__roastty_prompt_state" != prompt-start
            echo -en "\e]133;D\a"
        end

        set --global __roastty_prompt_state prompt-start
        echo -en $__roastty_prompt_start_mark
    end

    function __roastty_mark_output_start --on-event fish_preexec
        set --global __roastty_prompt_state pre-exec
        echo -en "\e]133;C\a"
    end

    function __roastty_mark_output_end --on-event fish_postexec
        set --global __roastty_prompt_state post-exec
    ...
    # Enable fish to handle reflow because Roastty clears the prompt on resize.
    set --global fish_handle_reflow 1

    # Initial calls for first prompt
    if string match -q 'cursor*' -- $features
        __roastty_set_cursor_beam
    end
    __roastty_mark_prompt_start
    __update_cwd_osc
end
